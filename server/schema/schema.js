...
//lodge's data
let lodges = [
    { id: '1', title: 'Apartment in center', typeOfInn: 'Apartment', rooms: { beds: 2, rooms: 1, bathRooms: 1 }, maxGuests: 4, city: 'London', country: 'UK', rating: 4, avaliability: true,hostId: '1' },
    { id: '2', title: 'Castle', typeOfInn: 'Private Room', rooms: { beds: 1, rooms: 100, bathRooms: 10 }, maxGuests: 2, city: 'Versales', country: 'FR', rating: 5, avaliability: false, hostId: '2'},
    { id: '3', title: 'Titanic trip', typeOfInn: 'Cruzer Room', rooms: { beds: 3, rooms: 1, bathRooms: 30 }, maxGuests: 2, city: 'Titanic', country: 'Ocean', rating: 1, avaliability: true, hostId: '3' },
]
//host's data
let hosts = [
...
const HostType = new GraphQLObjectType({
    name: 'Host',
    description: 'Host information, that contains the identifier for relationship with the lodges, and if it is a superhost',
    fields: () => ({
        id: { type: GraphQLString },
        name: { type: GraphQLString },
        rating: { type: GraphQLFloat },
        ...
        allLodges: {
            ...
            resolve(parent, args) {
               ...
               //filter by for each criteria as args are passed
                ...
                if (args.superHost !== undefined)
                    lodgesFiltered = filteredSuperHosts;
                if (args.title !== undefined)
                    lodgesFiltered = lodgesFiltered.filter(lodge => lodge.title === args.title);
                if (args.avaliability !== undefined)
                    lodgesFiltered = lodgesFiltered.filter(lodge => lodge.avaliability === args.avaliability);
